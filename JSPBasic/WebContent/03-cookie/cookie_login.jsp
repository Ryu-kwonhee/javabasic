...
<%
	...
	// cookie_welcome.jsp 상단의 처리 로직을 참고해주세요.
	// 추출한 아이디, 비밀번호를 콘솔창에 찍어주세요.
	String userId = "";
	String userPw = "";
		// cookies 변수에 쿠키가 여러 개 들어있음.
		// 반복문을 이용해 하나하나 cook에 번갈아가면서 한 번씩 대입해서 코드 진행
	System.out.println(cookies);
	System.out.println(cookies==null);
	
	for(Cookie c : cookies) {
			// 개별 쿠키의 이름이 cookieName 변수에 저장
			String cookieName = c.getName();
			// 저장한 쿠키의 이름이 "remember_id" 인 경우
			if (cookieName.equals("remember_id")) {
				// userPw 변수에 remember_id 변수에 저장해둔
				// 로그인 아이디를 저장
				userId = c.getValue();
			}
