...
public class OjdbcInsert {
	public static void main(String[] args) {
		...
		System.out.println("1번 컬럼에 넣을 숫자를 입력하세요.");
		int num = scan.nextInt();
		System.out.println("2번 컬럼에 넣을 문자를 입력하세요.");
		String string = scan.next();
		System.out.println("입력이 완료되었습니다.");
		// Statement를 대신할 수 있는 PreparedStatement 선언
		// 기존 Statement는 SQL구문과 자바 구문의 경계가 불분명했습니다.
		// 그래서 자바 코드와 SQL 구문이 섞여서 굉장히 지저분했는데 
		...
		try {
			...
			String sql = "INSERT INTO test(num, str) VALUES "+ "(?, ?)";
			
			// 2. 만든 쿼리문의 ?자리에 적용할 자바 변수를 집어넣습니다.
			// pst변수를 생성해놓은 다음, set자료형(?순서, 집어넣을 변수)를 이용해 ?를 모두 채워줍니다.
